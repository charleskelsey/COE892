...
class DataStore:
    def __init__(self):
        import os
        # Load map from map1.txt
        map_file = "map1.txt"
        if os.path.exists(map_file):
            with open(map_file, "r") as f:
                lines = [line.strip() for line in f if line.strip() and not line.startswith("//")]
            dims = lines[0].split()
            self.map_height = int(dims[0])
            self.map_width = int(dims[1])
            self.grid = []
            for line in lines[1:]:
        ...
        # Initialize mines from cells set to 1 in the grid using mines.txt
        self.mines = {}  # id -> Mine
        self.next_mine_id = 1
        mine_file = "mines.txt"
        mine_serials = []
        if os.path.exists(mine_file):
            with open(mine_file, "r") as f:
                mine_serials = [line.strip() for line in f if line.strip() and not line.startswith("//")]
        idx = 0
        # For each cell marked as 1 in the grid, create a mine using a serial from mines.txt
        for y in range(self.map_height):
            for x in range(self.map_width):
                if self.grid[y][x] == 1:
                    if idx < len(mine_serials):
                        serial = mine_serials[idx]
                        idx += 1
                    else:
                        serial = "default_serial"
                    # Create mine object (using Mine schema)
                    from fast_api_server import Mine  # ensure Mine is imported
                    new_mine = Mine(id=self.next_mine_id, x=x, y=y, serial_number=serial)
                    self.mines[self.next_mine_id] = new_mine
                    self.next_mine_id += 1
        
        # Initialize rovers as empty
...
async def update_mine(mine_id: int, mine_update: MineUpdate):
    ...
    old_x, old_y = mine.x, mine.y
    
    # Update mine fields if provided
    ...
    # Update grid
    db.update_grid_for_mine(mine_id, old_x, old_y)
