...
class RoverOperator:
    ...
    async def control_rover_realtime(self, rover_id):
        """Enhanced real-time rover control using WebSocket"""
        try:
            ...
            async with websockets.connect(uri) as websocket:
                ...
                # Fetch initial rover and map state
                try:
                    rover = self.get_rover(rover_id)
                    grid = self.get_map()
                except Exception as e:
                    print(f"Error fetching initial rover/map state: {e}")
                    return
            
                if not rover or not grid:
                    print("Unable to retrieve rover or map information.")
                    return
            
                # Initialize rover position tracking
                current_position = {
                    'x': 0,
                    'y': 0,
                    'facing': 'S'
                ...
                while True:
                    # Display current map and rover position
                    try:
                        self.display_map(grid, current_position)
                    except Exception as e:
                        print(f"Error displaying map: {e}")
                
                    # Get command from user
                    command = input("Enter command (L/R/M/D/q): ").upper()
                    ...
                    if command not in ['L', 'R', 'M', 'D']:
                        print("Invalid command. Use L, R, M, D, or q")
                        continue
                    ...
                    try:
                        ...
                        if response.get('status') == 'success':
                            ...
                            if 'newPosition' in response:
                                current_position['x'] = response['newPosition']['x']
                                current_position['y'] = response['newPosition']['y']
                                print(f"New position: ({current_position['x']}, {current_position['y']})")
                        
                            # Update facing direction if rotation occurred
                            if 'newFacing' in response:
                                current_position['facing'] = response['newFacing']
                                print(f"New facing direction: {current_position['facing']}")
                    
                        elif response.get('status') == 'warning':
                            print("WARNING: " + response.get('message', 'Unknown warning'))
                        
                            # Update position for warnings (like being on a mine)
                            if 'newPosition' in response:
                                current_position['x'] = response['newPosition']['x']
                                current_position['y'] = response['newPosition']['y']
                        ...
                        elif response.get('status') == 'eliminated':
                            print("ROVER ELIMINATED: " + response.get('message', 'Rover destroyed'))
                            break
                    
                        # Special handling for disarming mines
                        if command == 'D' and response.get('pin') is not None:
                            print(f"Mine disarmed! PIN: {response['pin']}")
    ...
    while True:
        ...
        if choice == '1':
            ...
            async def run_realtime_control():
                await operator.control_rover_realtime(rover_id)
            await run_realtime_control()
        elif choice == '9':
            break
        else:
            print("Invalid choice. Please try again.")
